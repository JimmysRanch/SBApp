  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const [staffResp, serviceResp, addOnResp, apptResp] = await Promise.all([
          supabase
            .from("employees")
            .select("id,name,initials,calendar_color_class,profile_slug")
            .eq("active", true)
            .order("name"),
          supabase
            .from("services")
            .select("id,name,duration_min,base_price,color_class,service_sizes(id,label,multiplier,sort_order)")
            .eq("active", true)
            .order("name"),
          supabase
            .from("add_ons")
          ...
          supabase
            .from("appointments")
            .select(
              "id,employee_id,service_id,service_size_id,start_time,end_time,discount,tax,status,notes,client:clients(first_name,last_name),pet:pets(name),appointment_add_ons(add_on_id)"
            )
        ...
        const staffData: StaffMember[] = (staffResp.data ?? []).map((row) => {
          const name: string = row.name ?? `Staff #${row.id}`;
          const generatedInitials = name
            ...
            .join("")
            .slice(0, 2);
          const fallbackInitials = generatedInitials || name.slice(0, 2).toUpperCase();
          const initials = (row.initials as string | null) ?? fallbackInitials;
          return {
            id: String(row.id),
            name,
            initials,
            profileId: (row.profile_slug as string | null) ?? `staff-${row.id}`,
            colorClass: (row.calendar_color_class as string | null) ?? "bg-brand-bubble/20 text-white",
          } satisfies StaffMember;
        });
        setStaffDirectory(staffData);

        const serviceData: DrawerServiceOption[] = (serviceResp.data ?? []).map((row) => ({
          id: String(row.id),
          name: row.name ?? "Service",
          basePrice: Number(row.base_price ?? 0),
          color: (row.color_class as string | null) ?? "bg-brand-bubble/20 text-white",
          sizes: ((row.service_sizes as any[]) ?? [])
