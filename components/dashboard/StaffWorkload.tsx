...
async function fetchAppointmentsForDay(startISO: string, endISO: string) {
  ...
  if (!attempted) {
    ...
    if (fallback.error) {
      if (isMissingRelationError(fallback.error) || isMissingColumnError(fallback.error)) {
        return []
      }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (shiftsRes.error) {
          if (isMissingRelationError(shiftsRes.error) || isMissingColumnError(shiftsRes.error)) {
