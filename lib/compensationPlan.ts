...
export function planSummaryLines(plan: CompensationPlan, options: SummaryOptions = {}): string[] {
  ...
  const guaranteeHigherActive =
    plan.guarantee.enabled &&
    plan.guarantee.payout_mode !== "stacked" &&
    Number.isFinite(plan.guarantee.weekly_amount) &&
    (plan.guarantee.weekly_amount ?? 0) > 0;

  if (
    plan.commission.enabled &&
    Number.isFinite(plan.commission.rate) &&
    (plan.commission.rate ?? 0) > 0 &&
    !guaranteeHigherActive
  ) {
